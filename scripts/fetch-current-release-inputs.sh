...
while read -r run_id head; do
  ...
  response=$(gh api "repos/$GITHUB_REPOSITORY/actions/runs/$run_id" \
    --jq '[.status,.conclusion,.head_sha,.head_branch,.run_attempt,.repository.id] | @tsv')
  IFS=$'\t' read -r status conclusion observed branch attempt repository_id <<< "$response"
  [[ $status == completed && $conclusion == success && $observed == "$head" &&
     $branch == feat/hosted-capacity-probe-20260923 && $attempt == 1 &&
     $repository_id == 1383172221 ]] || {
    echo "fixed release evidence run $run_id differs" >&2
    exit 1
  }
done <<'RUNS'
35935470213 197be7276c00fb9d1eb89b4739eec6dcec9e6b14
35992034723 4faf67a03d87d1a6598790d4f690aceaf39df432
35992359796 e0de0feccb93b86e86c8d304efe781ba9c3be69a
35992468711 b7a130c9452a06c31d62541203bea101b22b4445
35992525759 e6bdb1ad55706ab70ae002cef62e296e39f304c7
35992626604 7c31d1bae491e7e6d126b8a962df62ede74d7027
35992749716 f90ae9da2527d1e46b79e18a15b4eb28601e7add
36028598666 e1c21547e7ba4b6938084f8434e19ced4a339b57
RUNS
