#!/usr/bin/python3 -I
"""Bind a fresh Trivy report to the fixed unsigned v25 hosted voice image.
...
IMAGE_SHA256 = '87d7b185bfd23177c923d19cb3ee07b769d600d493a48f616d9eca729000ff63'
APP_REVISION = '38542697a44b5314b6cfe73d17f7f7548ff57bf6'
IMAGE_NAME = 'teleagent-v25-voice-image.docker.tar'
...
def verify(image, report_path, metadata_path, now):
    ...
    with tarfile.open(image, 'r:') as archive:
        ...
        need(type(manifest) is list and len(manifest) == 1 and
             manifest[0].get('RepoTags') == ['teleagent-v25-voice-diagnostic:20260926'],
             'image tag differs')
    ...
    sqlite = [row for row in by_kind[('lang-pkgs', 'node-pkg')]['Packages']
              if row.get('Name') == 'better-sqlite3']
    need(len(sqlite) == 1 and sqlite[0].get('Version') == '13.0.3',
         'N-API SQLite package absent from scanner inventory')
